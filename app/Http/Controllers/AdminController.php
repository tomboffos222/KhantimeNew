...
class AdminController extends Controller
{
    public  function Posts(){
        $data['user_counter']  = Counter::count();
        $data['counter'] = Counter::pluck('counter')->sum();
        $data['cats'] = Article_category::get();
        $data['articles']= Article::orderBy('id','desc')->paginate(10);
        $data['total'] = Article::pluck('views')->sum();
    ...
    public function DeletePost($articleId){
        ...
        $this->deleteFile(public_path($article['path']));
        $article->delete();
    ...
    public function ArticleCategories(){
        $data['articles'] = Article_category::paginate(25);
        $data['sub_cats'] = Sub_category::paginate(25);
        return view('admin.categories',$data);
    }
    public function DeleteSubCategory($id){
        $cat = Sub_category::find($id);
        $cat->delete();

        return back()->with('message','Удалено');
    }
    public function DeleteCategory($id){
        $article = Article_category::find($id);
        $article->delete();
        return back()->with('message','Удалено');
    }
    ...
    public function StoreArticle( Request $request){
        ...
        if ($validator->fails()){
            ...
            if ($request->hasFile('path')) {
                ...
                $article['views'] = 0;
                if($request['sub_cat']){
                    $article['sub_cat_id'] = $request['sub_cat'];
                }
    ...
    public function SubCat(Request $request){
        ...
        $messages = [
            ...
            'parent_id.required' => 'Выберите категорию'
        ];
        $validator = $this->validator($request->all(),$rules, $messages);
        if ($validator->fails()){
            return back()->withErrors($validator->errors());
            ...
            return back()->with('message','Успешно создано');
        }
