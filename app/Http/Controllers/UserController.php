...

{
    ...
    public function SearchForm(Request $request){
        ...
        if ($validator->fails()){
            ...
            $data['sub_cats'] = Sub_category::get();
    ...
    public function Home(){
        ...
        $counters = Counter::where('title', $visitor_ip)->first();
        if (!$counters){
            $counter = new Counter();
            $counter['title'] = $visitor_ip;
            $counter['counter'] +=1;
            $counter->save();
        }else{
            $counters['counter'] +=1;
        ...
        $data['popularArticle'] = Article::join('article_categories','articles.cat_id','=','article_categories.id')->select('articles.*','name')->orderBy('views','desc')->first();
        $data['articles'] = Article::join('article_categories','articles.cat_id','=','article_categories.id')->select('articles.*','name')->where('slide',1)->get();
        $data['NewArticles'] = Article::join('article_categories','articles.cat_id','=','article_categories.id')->select('articles.*','name')->orderBy('created_at','desc')->paginate(4);
        $data['cats'] = Article_category::get();
        $data['sub_cats'] = Sub_category::get();
    ...
    public  function Sub($categoryId){
        $data['popularArticle'] = Article::join('article_categories','articles.cat_id','=','article_categories.id')->select('articles.*','name')->orderBy('views','desc')->first();
        $data['cats'] = Article_category::get();
        $data['mainCat'] = Sub_category::find($categoryId);
        $data['sub_cats'] = Sub_category::get();
        $cat = Sub_category::find($categoryId);
        $data['articles'] = Article::join('article_categories','articles.cat_id','=','article_categories.id')->select('articles.*','name')->orderBy('views','desc')->paginate(4);
        $data['articlesMain'] = Article::join('article_categories','articles.cat_id','=','article_categories.id')->select('articles.*','name')->where('sub_cat_id',$cat['id'])->orderBy('created_at','desc')->paginate(8);
        $data['NewArticles'] = Article::join('article_categories','articles.cat_id','=','article_categories.id')->select('articles.*','name')->orderBy('created_at','desc')->paginate(4);
        return view('article_category',$data);
    }
